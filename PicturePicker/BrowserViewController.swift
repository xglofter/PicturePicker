...
extension BrowserViewController {
    ...
    func tapSelectFlagView() {
        let number = PickerManager.shared.getPhotoNumber(with: photos[currentPageIndex].asset.localIdentifier)
        if number == 0 { // 未被选中的图片
            ...
            PickerManager.shared.choosePhoto(with: photos[currentPageIndex].asset)
            touch(with: PickerManager.shared.currentNumber)
            
        } else { // 已被选中的图片
            PickerManager.shared.unchoosePhoto(with: photos[currentPageIndex].asset.localIdentifier)
            touch(with: 0)
        }
    }

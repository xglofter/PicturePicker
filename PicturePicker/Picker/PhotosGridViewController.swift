...
extension PhotosGridViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // 获取 flag
        let number = PicturePicker.shared.getPhotoNumber(with: asset.localIdentifier)
        cell.setFlag(isSelected: (number != 0), with: number)
        
        // 点击 flag 回调处理
        cell.flagCallback = { [weak self] in

            let number = PicturePicker.shared.getPhotoNumber(with: cell.asset.localIdentifier)
            if number == 0 { // 未被选中的图片
                if PicturePicker.shared.isMax {
                    self?.alert(with: "您最多只能选\(PicturePicker.shared.maxPhotos)张")
                    return
                }
                
                PicturePicker.shared.choosePhoto(with: cell.asset)
                cell.touch(with: PicturePicker.shared.currentNumber)
                
            } else { // 已被选中的图片
                PicturePicker.shared.unchoosePhoto(with: cell.asset.localIdentifier)
                cell.touch(with: 0)
...
private extension PhotosGridViewController {
    ...
    func checkBarShouldHidden() {
        let number = PicturePicker.shared.currentNumber
        ...
        toolBar.setCurrentNumber(number: number, maxNumber: PicturePicker.shared.maxPhotos)
    }
